...
async def global_filters(client, message, text=False):
    ...
    for keyword in reversed(sorted(keywords, key=len)):
        ...
        return False
# Patch to add spell-check + auto-match feature to pm_filter.py
...
# Function to handle user text and respond with matched file or suggestions
async def handle_spellcheck_or_send(bot, message, query):
    from database.ia_filterdb import get_search_results, get_file_details
    from utils import get_size

    files, _, _ = await get_search_results(chat_id=message.chat.id, query="", offset=0, filter=True)
    matches = []

    for file in files:
        score = get_match_score(query, file.file_name)
        if score >= 80:
            await bot.send_cached_media(
                chat_id=message.chat.id,
                file_id=file.file_id,
                caption=f"\ud83c\udfac {file.file_name} (Match: {score}%)"
            )
            return
        elif score >= 50:
            matches.append((file.file_name, file.file_id, score))

    if matches:
        matches = sorted(matches, key=lambda x: x[2], reverse=True)[:5]
        buttons = [
            [InlineKeyboardButton(f"{m[0]} ({m[2]}% match)", callback_data=f"spellmatch#{m[1]}")]
            for m in matches
        ]
        buttons.append([InlineKeyboardButton("\u274c Close", callback_data="close_data")])
        await message.reply_text("\ud83d\udcfa Did you mean one of these ?", reply_markup=InlineKeyboardMarkup(buttons))
    else:
        await message.reply_text("\ud83d\ude15 No similar files found.")

# Callback to send the selected file when suggestion is clicked
@Client.on_callback_query(filters.regex(r"^spellmatch#"))
async def send_matched_file(bot, query):
...
# In your pm_text or give_filter or auto_filter handler, call this like:
# await handle_spellcheck_or_send(bot, message, message.text)
